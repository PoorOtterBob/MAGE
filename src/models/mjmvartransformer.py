...
class STID(BaseModel):
    def __init__(self, **args):
        ...
        self.encoder = nn.Sequential(
            *[SelfAttentionLayer(self.hidden_dim, 4*self.hidden_dim) for _ in range(self.num_layer)])
        '''self.encoder = nn.Sequential(
            *[STIDMultiLayerPerceptron(self.hidden_dim, self.hidden_dim) for _ in range(self.num_layer)])
        self.attn_layers_s = nn.ModuleList(
            [
                SelfAttentionLayer(self.hidden_dim, 4*self.hidden_dim)
...
class SelfAttentionLayer(nn.Module):
    def __init__(
        self, model_dim, feed_forward_dim=2048, num_heads=8, dropout=0.1, mask=False
    ):
        super().__init__()
