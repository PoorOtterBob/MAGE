...
class STID(BaseModel):
    def __init__(self, **args):
        ...
        self.input_len = 12
        self.input_dim = 3
        self.embed_dim = 64 # 64
        self.output_len = 12
        self.num_layer = 3
        self.temp_dim_tid = 64
        self.temp_dim_diw = 64
        self.temp_dim_miy = 64
        '''
        self.num_nodes = self.node_num
        self.node_dim = 32
        self.input_len = 12
        ...
        # node = self.node_emb
        # day = self.time_in_day_emb
        # week = self.day_in_week_emb
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/node.npy', node.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/day.npy', day.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/week.npy', week.detach().cpu().numpy())
        # print('Prompt done')
...
class STRIP(BaseModel):
    def __init__(self, model_args, stmodel, dim, core, **args):
        ...
        if self.kernel_num: 
            ...
            # alpha = self.alpha
            # beta = self.beta
            # np.save('/home/mjm/LST/LargeST-main/AAAI2025/alpha.npy', alpha.detach().cpu().numpy())
            # np.save('/home/mjm/LST/LargeST-main/AAAI2025/beta.npy', beta.detach().cpu().numpy())
            # print('a b done! ')
    ...
    def adaptive_adj_generation(self):
        ...
        # ada = adaptive_kernel
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/ada.npy', ada.detach().cpu().numpy())
        # print('Ada Done!')
        # sys.exit()
...
class Core_Adaptive(nn.Module):
    ...
    def forward(self, input, adj=None, *args, **kwargs): 
        ...
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/core_emb.npy', self.cores.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/node_emb.npy', self.adpative.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/affiliation.npy', affiliation.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/affiliation_node_to_core.npy', affiliation_node_to_core.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/affiliation_core_to_node.npy', affiliation_core_to_node.detach().cpu().numpy())
        # print(" Core Done!")
        # sys.exit()
