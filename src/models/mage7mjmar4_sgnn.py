...
class MAGE_Block(nn.Module):
    def __init__(self, dim, expert_num, node_num, depth, model_dim, graph_gen_dim=32, head=4):
        ...
        self.router = nn.Sequential(
            nn.Linear(dim, expert_num, bias=False),
            nn.Softmax(dim=-1)
        )
        
        # self.filter = nn.Parameter(torch.rand(2, expert_num, node_num))
        self.filter = nn.Sequential(
            nn.Linear(dim, 2, bias=False),
            # nn.Softmax(dim=-2),
            # nn.ELU(alpha=1),
            nn.Sigmoid()
        )
    ...
    def reset_parameter(self):
        nn.init.constant_(self.filter[0].weight, 0)
        if self.filter[0].bias is not None:
            nn.init.constant_(self.filter[0].bias, 0)
    ...
    def forward(self, x):
        ...
        # x = torch.einsum('bik, e, ekid, bje, ekdj, bjf -> bif', 
                         self.router(x),
                         self.cal_coefficient(),
                         torch.softmax(self.E1, dim=-1),
                         # torch.softmax(self.filter, dim=-1)*self.node_num,
                         # self.filter(x).squeeze(-1), #*self.node_num,
                         # self.filter(x)*2, #*self.node_num,
