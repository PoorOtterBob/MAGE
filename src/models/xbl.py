...
class STID(BaseModel):
    def __init__(self, **args):
        ...
        self.time_of_day_size = 96 # may be 96
        ...
        if self.if_time_in_day:
            self.time_in_day_emb1 = nn.Parameter(
                torch.empty(self.time_of_day_size, self.temp_dim_tid))
            nn.init.xavier_uniform_(self.time_in_day_emb1)
            self.time_in_day_emb2 = nn.Parameter(
                torch.empty(self.time_of_day_size, self.temp_dim_tid))
            nn.init.xavier_uniform_(self.time_in_day_emb2)
        if self.if_day_in_week:
            self.day_in_week_emb1 = nn.Parameter(
                torch.empty(self.day_of_week_size, self.temp_dim_diw))
            nn.init.xavier_uniform_(self.day_in_week_emb1)
            self.day_in_week_emb2 = nn.Parameter(
                torch.empty(self.day_of_week_size, self.temp_dim_diw))
            nn.init.xavier_uniform_(self.day_in_week_emb2)
        '''
        # embedding layer
    ...
    def forward(self, history_data, label=None, adj=None):
        ...
        if self.if_time_in_day:
            ...
            time_in_day_emb1 = self.time_in_day_emb1[(t_i_d_data[:, -1, :] * self.time_of_day_size).type(torch.LongTensor)]
            time_in_day_emb2 = self.time_in_day_emb2[(t_i_d_data[:, -1, :] * self.time_of_day_size).type(torch.LongTensor)]
        else:
            time_in_day_emb = None
        if self.if_day_in_week:
            d_i_w_data = history_data[..., 2]
            day_in_week_emb1 = self.day_in_week_emb1[(d_i_w_data[:, -1, :] * self.day_of_week_size).type(torch.LongTensor)]
            day_in_week_emb2 = self.day_in_week_emb2[(d_i_w_data[:, -1, :] * self.day_of_week_size).type(torch.LongTensor)]
        else:
        ...
        '''
        seasonal_init, trend_init = self.decompsition(input_data[..., 0])
        ...
        if self.if_spatial:
            ...
            node_emb1.append(self.node_emb1.unsqueeze(0).expand(
                batch_size, -1, -1).transpose(1, 2).unsqueeze(-1))
            node_emb2.append(self.node_emb2.unsqueeze(0).expand(
                batch_size, -1, -1).transpose(1, 2).unsqueeze(-1))
        # temporal embeddings
        tem_emb1 = []
        tem_emb2 = []
        if time_in_day_emb is not None:
            tem_emb1.append(time_in_day_emb1.transpose(1, 2).unsqueeze(-1))
            tem_emb2.append(time_in_day_emb2.transpose(1, 2).unsqueeze(-1))
        if day_in_week_emb is not None:
            tem_emb1.append(day_in_week_emb1.transpose(1, 2).unsqueeze(-1))
            tem_emb2.append(day_in_week_emb2.transpose(1, 2).unsqueeze(-1))

        # concate all embeddings
        hidden1 = torch.cat([seasonal_output.unsqueeze(-1)] + node_emb1 + tem_emb1, dim=1)
        hidden2 = torch.cat([trend_output.unsqueeze(-1)] + node_emb2 + tem_emb2, dim=1)
        h = (hidden1+hidden2).transpose(1, -1)
        # encoding
        hidden1 = self.encoder1(hidden1)
        hidden2 = self.encoder2(hidden2)
        z = (hidden1+hidden2).transpose(1, -1)
        # regression
        prediction = self.regression_layer(hidden1+hidden2)
    ...
    def __init__(self, model_args, stmodel, dim, core, **args):
        ...
        if core:
            ...
            self.backcast = nn.Sequential(
                nn.Linear(self.in_dim, self.backcast_hidden_dim), 
                nn.GELU(),
                nn.Linear(self.backcast_hidden_dim, self.out_dim), 
            )
