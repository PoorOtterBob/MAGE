...
class temporalEmbedding(nn.Module):
    def __init__(self, D, tod, dow=7):
        super(temporalEmbedding, self).__init__()
        self.ff_te = FeedForward([tod + dow, D, D])
...
class STWave(BaseModel):
    ...
    def __init__(self, 
        ...
        super(STWave, self).__init__(**args)
        ...
        self.start_emb_l = FeedForward([input_dim-2, hidden_size, hidden_size])
        self.start_emb_h = FeedForward([input_dim-2, hidden_size, hidden_size])
        self.te_emb = temporalEmbedding(hidden_size, time_in_day_size, day_in_week_size)
        ...
        self.end_emb = FeedForward([hidden_size, hidden_size, output_dim])
        self.end_emb_l = FeedForward([hidden_size, hidden_size, output_dim])
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor):
        ...
        '''
        x = history_data
        te = torch.cat([x[:, :, 0, -2].unsqueeze(-1)*self.td, x[:, :, 0, -1].unsqueeze(-1)*self.dw], -1)
        ...
        inputs = x[..., :self.id]
        xl, xh = disentangle(inputs[..., :-2].cpu().numpy(), self.wt, self.wl)
