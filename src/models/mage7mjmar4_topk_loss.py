...
class MAGE_Block(nn.Module):
    def __init__(self, dim, expert_num, node_num, depth, model_dim, graph_gen_dim=32, head=4):
        ...
        # Differential
        # self.E1 = nn.Parameter(torch.rand(expert_num, 2, node_num, graph_gen_dim))
        # self.E2 = nn.Parameter(torch.rand(expert_num, 2, graph_gen_dim, node_num))
        self.E1 = [nn.Parameter(torch.rand(2, node_num, graph_gen_dim)) \
                   for _ in range(expert_num)]
        self.E2 = [nn.Parameter(torch.rand(2, graph_gen_dim, node_num)) \
                   for _ in range(expert_num)]
        ...
        # self.outer = nn.Parameter(torch.zeros((expert_num, dim, dim), 
        #                                       dtype=torch.float32).normal_(mean=0,std=0.1))
        self.outer = [nn.Parameter(torch.eye(dim)) \
                      for _ in range(expert_num)]
        # self.bias = nn.Parameter(torch.zeros(expert_num))
    ...
    def forward(self, x):
        ...
        shape = x.size()
        x = x.view(-1, self.dim)
        route, indices = self.loss_free_route(x)
        out = torch.zeros_like(x)
        counts = torch.bincount(indices.flatten(), 
                                minlength=self.expert_num).tolist()
        for i, (E1, E2, W) in enumerate(zip(self.E1, self.E2, self.outer)):
            if counts[i] == 0:
                continue
            idx, top = torch.where(indices == i)
        x = torch.einsum('bik, e, ekid, ekdj, bjf, kfc -> bic', 
                         route,
                         ...
                         x,
                         self.outer)
        return x # self.outer(x) # x

    def adaptive_graph_convolution(self, x, E1, E2, W):
        x = torch.einsum('e, eid, edj, bjf, fc -> bic', 
                         self.cal_coefficient(),
    ...
    def loss_free_route(self, x, K=4):
        route = self.router(x)
        _, indices = torch.topk(route + self.bias, K, dim=-1) # (b*n, k)
        route = torch.gather(route, dim=1, index=indices) #  (b*n, k, d)
