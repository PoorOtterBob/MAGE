...
class DeepAir(BaseModel):
    def __init__(self, d_hid, if_future_side_information, **args):
        ...
        self.FusionNet = nn.ModuleList([
            FusionNet(d_hid*((self.input_dim + ((self.input_dim - 1) if if_future_side_information else 0)) if i == 0 else 2))
        for i in range(self.input_dim + ((self.input_dim - 1) if if_future_side_information else 0))])
        
        self.emb = nn.ModuleList([
            nn.Conv1d(self.seq_len, 
                      d_hid, 
                      kernel_size=1)
        for _ in range(self.input_dim + ((self.input_dim - 1) if if_future_side_information else 0))])

        self.merge = nn.ModuleList([
            nn.Conv1d(d_hid*((self.input_dim + ((self.input_dim - 1) if if_future_side_information else 0)) if i == 0 else 2), 
    ...
    def forward(self, x, label=None, adj=None):
        ...
        if self.if_future_side_information:
            for i in range(self.input_dim-1): 
                h = self.emb[self.input_dim+i](label[..., i])
                feature.append(h)
                z = self.FusionNet[self.input_dim+i](torch.cat([h_x, h], dim=1))
                output += self.merge[self.input_dim+i](z)
