...
class Model_backbone(BaseModel):
    def __init__(self, 
                 # c_in:int, 
                 # seq_len:int, 
                 # pred_len:int, 
                 # patch_len:int=24, 
                 stride:int=24, n_layers:int=3, d_model=128, n_heads=16, d_ff:int=256, 
                 ...
                 QAM_end:float =0.5, padding_patch = None, **args):
        
        super(Model_backbone, self).__init__(**args)
    ...
    def forward(self, z, label=None):                                                                   # z: [bs x nvars x seq_len]
        ###
        z = z[..., 0].transpose(1, -1)
        ###
