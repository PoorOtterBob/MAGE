...
class MAGE_Block(nn.Module):
    def __init__(self, dim, expert_num, node_num, depth, model_dim, graph_gen_dim=32, head=4, topk=4):
        ...
        self.lambda_q1 = nn.Parameter(torch.empty(1, expert_num,  model_dim//expert_num, dtype=torch.float32).normal_(mean=0,std=0.1))
        self.lambda_k1 = nn.Parameter(torch.empty(1, expert_num,  model_dim//expert_num, dtype=torch.float32).normal_(mean=0,std=0.1))
        self.lambda_q2 = nn.Parameter(torch.empty(1, expert_num,  model_dim//expert_num, dtype=torch.float32).normal_(mean=0,std=0.1))
        self.lambda_k2 = nn.Parameter(torch.empty(1, expert_num,  model_dim//expert_num, dtype=torch.float32).normal_(mean=0,std=0.1))

        self.coefficient = nn.Parameter(torch.ones(expert_num), requires_grad=False)
    ...
    def cal_coefficient(self):
        # return torch.concat([self.coefficient, self.cal_lambda()], dim=0) # (2, k)
        return F.pad(self.cal_lambda(), (0, 0, 1, 0), mode='constant', value=1)
    ...
    def selector(self, x, bias=0):
        ...
        route_mask = self.normer(route + mask.log())
    ...
    def forward(self, x, bias):
        ...
        x = torch.einsum('bik, ek, ekid, ekdj, bjf, fc -> bic', 
                         route,
