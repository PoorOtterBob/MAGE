...
class DGCRN(BaseModel):
    ...
    def __init__(self, device, predefined_adj, gcn_depth, rnn_size, hyperGNN_dim, node_dim, \
        ...
        dims_hyper = [self.hidden_size + max(self.input_dim - 2, 3), hyperGNN_dim, middle_dim, node_dim]
        ...
        dims = [max(self.input_dim - 2, 3) + self.hidden_size, self.hidden_size]
    ...
    def forward(self, input, label=None, batches_seen=None, task_level=12):  # (b, t, n, f)
        ...
        timeofday = self.compute_future_info(x[:, -2: ,: ,  :])
