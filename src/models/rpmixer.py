...
class RPMixer(BaseModel):
    ...
    def __init__(self, 
                 ...
                 is_freq='True', 
                 **args):
        super(RPMixer, self).__init__(**args)
        self.model_name = 'RPMixer'
        seq_len = self.seq_len
        pred_len = self.horizon
        seq_dim = self.node_num
        feat_dim = self.input_dim - 1
    ...
    def forward(self, x, label=None):
        feat = x[:, -1, :, 1:].transpose(1, -1) # -> (b, f-1, n)
        x = torch.cat([x[..., 0], feat], dim=1).transpose(1, -1) # (b, n, t+f-1)
