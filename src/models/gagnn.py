...
class GAGNN(BaseModel):
	def __init__(self,
		...
		super(GAGNN, self).__init__(**args)
		self.TIME_WINDOW = self.seq_len
		self.device = device
		self.mode = mode
		...
		self.edge_index = torch.tensor(edge_index, dtype=torch.int64, requires_grad=False).to(device)
		self.edge_w = torch.tensor(edge_w, dtype=torch.float, requires_grad=False).to(device)
		self.loc = torch.tensor(loc, dtype=torch.float, requires_grad=False).to(device)

		if self.encoder == 'self':
			try:
				self.encoder_layer = TransformerEncoderLayer(self.input_dim, nhead=4, dim_feedforward=256)
			except:
				self.encoder_layer = TransformerEncoderLayer(self.input_dim, nhead=1, dim_feedforward=256)
			# self.x_embed = Lin(8, x_em)
			self.x_embed = Lin(self.TIME_WINDOW*self.input_dim, x_em)
		...
		self.edge_inf = Seq(Lin(x_em*2+date_em*2+loc_em*2,edge_h),ReLU(inplace=True))
	...
	def forward(self, x, label=None):
		...
		if self.encoder == 'self':
			...
			x = x.transpose(0,1)
			# print(x.shape)
			x = x.reshape(-1,self.city_num,self.TIME_WINDOW*x.shape[-1])
			x = self.x_embed(x)
		...
		# print(loc.shape)
		loc = self.loc_embed(self.loc).unsqueeze(0).repeat(x.shape[0], 1, 1)
		x_loc = torch.cat([x, loc], dim=-1)
		g_x = torch.bmm(w1, x_loc)
		# print(g_x.shape)

		# group gnn
		# u_em1 = self.u_embed1(u[:,-3])
		u_em2 = self.u_embed2((u[:,-1]*7).long())
		u_em3 = self.u_embed3((u[:,-2]*24).long())
		# u_em = torch.cat([u_em1,u_em2,u_em3],dim=-1)
		u_em = torch.cat([u_em2, u_em3], dim=-1)
		# print(u_em.shape)
		...
		# print(res.shape)
		return res.transpose(1, 2).unsqueeze(-1)
