...
class STID(BaseModel):
    ...
    def forward(self, history_data, label=None, adj=None):
        np.save('/home/mjm/LST/LargeST-main/AAAI2025/Draw/knowair_emb/day.npy', 
                self.time_in_day_emb.detach().cpu().numpy())
        np.save('/home/mjm/LST/LargeST-main/AAAI2025/Draw/knowair_emb/week.npy', 
                self.day_in_week_emb.detach().cpu().numpy())
        np.save('/home/mjm/LST/LargeST-main/AAAI2025/Draw/knowair_emb/node.npy', 
                self.node_emb.detach().cpu().numpy())
        sys.exit()
        ...
        if self.if_time_in_day:
            ...
            # print(t_i_d_data*self.time_of_day_size)
        ...
        seasonal_output = self.time_series_emb_layer1(seasonal_init.transpose(-1, -2).contiguous().view(
            batch_size, -1,  num_nodes))
        trend_output = self.time_series_emb_layer2(trend_init.transpose(-1, -2).contiguous().view(
            batch_size, -1, num_nodes))
...
class Core_Adaptive(nn.Module):
    ...
    def forward(self, input, adj=None, *args, **kwargs): 
        ...
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/com.npy', v.detach().cpu().numpy())
        # np.save('/home/mjm/LST/LargeST-main/AAAI2025/per.npy', (input-v).detach().cpu().numpy())
        # sys.exit()
