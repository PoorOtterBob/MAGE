...
class PatchSTG(BaseModel):
    def __init__(self, 
        ...
        super(PatchSTG, self).__init__(**args)
        ...
        # input_emb
        self.input_st_fc = nn.Conv2d(in_channels=self.input_dim, out_channels=input_dims, kernel_size=(1, tem_patchsize), stride=(1, tem_patchsize), bias=True)
        # spa_emb
    ...
    def forward(self, input, label):
        ...
        orginal = torch.zeros(rex.shape[0],
                              rex.shape[1],
                              self.node_num,rex.shape[-1]).to(x.device)
