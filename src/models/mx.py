...
class STRIP(BaseModel):
    def __init__(self, model_args, stmodel, dim, core, **args):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(4*self.decoder_hidden_dim, self.horizon),  
        )
...
class Core_Adaptive(nn.Module):
    def __init__(self, d_in, d_core, d_out, node_num, core_num, nndropout=0.3, dropout=0.08):
        ...
        self.adpative = nn.Parameter(torch.empty(d_core, node_num))
        self.cores = nn.Parameter(torch.empty(core_num, d_core))
        # self.affiliation = nn.Parameter(torch.empty(core_num, node_num))
    ...
    def forward(self, input, adj=None, *args, **kwargs): 
        ...
        q = self.query(input)
        affiliation = torch.einsum('cf, btnf -> btcn', self.cores, q) / self.d_core**0.5
        
        input = input.permute(0, 3, 1, 2)

        # affiliation = self.affiliation
        affiliation_node_to_core = torch.softmax(affiliation, dim=-1) # (b, t, c, n)
        affiliation_core_to_node = torch.softmax(affiliation, dim=-2) # (b, t, c, n)
        ...
        v = self.value(input)
        v = torch.einsum('bftn, btcn -> bftc', v, affiliation_node_to_core)
        v = torch.einsum('bftc, btcn -> bftn', v, affiliation_core_to_node)
