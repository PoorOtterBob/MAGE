...
def data_transformation_4_xformer(history_data: torch.Tensor, future_data: torch.Tensor, start_token_len: int):
    ...
    x_mark_enc = history_data[:, :, 0, -2:] - 0.5    # B, L1, C-1
...
class TimeMixer(BaseModel):
    ...
    def __init__(self, model_args, **args):
        super(TimeMixer, self).__init__(**args)
        ...
        self.channel_independence = model_args['channel_independence']
        self.d_model = model_args['d_model']
        self.dec_in = self.node_num
        self.enc_in = self.node_num
        self.c_out = self.node_num
    ...
    '''def forward(self, 
                ...
                train: bool,
                **kwargs) -> torch.Tensor:'''
    def forward(self, history_data, future_data=None):
