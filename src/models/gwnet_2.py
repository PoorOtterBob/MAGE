...
class GWNET(BaseModel):
    ...
    def __init__(self, supports, adp_adj, dropout, residual_channels, dilation_channels, \
        ...
        if adp_adj:
            self.nodevec1 = nn.Parameter(torch.full((self.node_num, 1), self.node_num**-0.5), requires_grad=False)
            self.nodevec2 = nn.Parameter(torch.full((1, self.node_num), self.node_num**-0.5), requires_grad=False)
            self.supports_len += 1
