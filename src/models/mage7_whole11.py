...
class MAGE(BaseModel):
    def __init__(self, model_args, **args):
        ...
        self.backbone1 = Pre_Norm_Transformer(self.args.model_dim, self.args.recur_num, self.node_num, blocknum=1)
        self.backbone2 = Pre_Norm_Transformer(self.args.model_dim, self.args.recur_num, self.node_num, blocknum=2)
        self.backbone3 = Pre_Norm_Transformer(self.args.model_dim, self.args.recur_num, self.node_num, blocknum=3)
...
class MAGE_Block(nn.Module):
    def __init__(self, dim, expert_num, node_num, depth, model_dim, graph_gen_dim=32, head=4):
        ...
        self.outer1 = nn.Parameter(torch.empty((expert_num, dim, dim)).normal_(mean=0,std=0.1))
        self.outer2 = nn.Parameter(torch.stack([torch.eye(dim) for _ in range(expert_num)], 
                                               dim=0), 
                                   requires_grad=False)
        self.outer_lambda = nn.Parameter(torch.stack([torch.tensor([[99]]).log() for _ in range(expert_num)], 
                                                     dim=0))
    ...
    def cal_outer(self):
        outer_weight = torch.sigmoid(self.outer_lambda/self.depth)
        # print(self.outer2.shape)
