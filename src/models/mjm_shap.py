...
class STID(BaseModel):
    ...
    def forward(self, history_data, label=None, adj=None):
        ...
        return  h, z, prediction, s, t, tem_emb[0], tem_emb[1], node_emb[0]
    ...
    def shap(self, flucts, stabs, days, weeks, nodes):
        seasonal_output = self.time_series_emb_layer1(flucts)
        trend_output = self.time_series_emb_layer2(stabs)

        time_series_emb = (seasonal_output + trend_output).unsqueeze(-1)

        # concate all embeddings
        hidden = torch.cat([time_series_emb] + [nodes] + [days, weeks], dim=1)
        # h = hidden.transpose(1, -1)
        # encoding
        hidden = self.encoder(hidden)
        # z = hidden.transpose(1, -1)
...
class STIDMultiLayerPerceptron(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        # hidden = self.fc2(self.drop(F.relu(self.fc1(input_data))))
...
class STRIP(BaseModel):
    ...
    def forward(self, x, label=None): 
        if self.extra_type:
            h, z, y, s, t, d, w, n = self.stmodel(x, label)
            ...
            return s, t, y_res.transpose(1, -1), y + y_res.transpose(1, -1), d, w, n
        else:
            return self.stmodel(x, label)
    
    def shap(self, flucts, stabs, corrs, errors, days, weeks, nodes):
            y = self.stmodel.shap(flucts, stabs, days, weeks, nodes)
            return y + corrs + errors
...
class STRIP_shap(BaseModel):
    def __init__(self, model_args, stmodel, dim, core, **args):
        super(STRIP_shap, self).__init__(**args)
        ## base spatio-temporal model
        self.stmodel = stmodel
        # self.inverse = stmodel.if_inverse()
    ...
    def forward(self, x, label=None): 
        if self.extra_type:
            ...
            return self.stmodel(x, label)
    '''
    # def forward(self, flucts, stabs, corrs, errors, days, weeks, nodes):
    def forward(self, flucts, stabs, corrs, errors, days, weeks, nodes):
            # flucts, stabs, corrs, errors, days, weeks, nodes = *input
            y = self.stmodel.shap(flucts, stabs, days, weeks, nodes)
            return torch.mean(torch.mean((y + corrs + errors).unsqueeze(-1), dim=2), dim=0, keepdim=True) / 20.
