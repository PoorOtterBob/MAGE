...
class MAGE(BaseModel):
    ...
    def forward(self, x, label=None, bias=[0, 0, 0]):
        ...
        x, topk_indices1 = self.backbone1(x, bias[0])
        x, topk_indices2 = self.backbone2(x, bias[1])
        x, topk_indices3 = self.backbone3(x, bias[2])
        x = self.decoder(x)
        if self.training:
...
class MAGE_Block(nn.Module):
    def __init__(self, dim, expert_num, node_num, depth, model_dim, graph_gen_dim=32, head=4, topk=4):
        ...
        # Differential
        self.E1 = nn.Parameter(torch.rand(2, expert_num+1, node_num, graph_gen_dim))
        self.E2 = nn.Parameter(torch.rand(2, expert_num+1, graph_gen_dim, node_num))
    ...
    def selector(self, x, bias=0):
        ...
        _, topk_indices = torch.topk(self.normer(route + bias), self.topk, dim=-1)
    ...
    def forward(self, x, bias):
        ...
        x = torch.einsum('bik, e, ekid, ekdj, bjf, fc -> bic', 
                         F.pad(route, (1, 0, 0, 0), mode='constant', value=1),
