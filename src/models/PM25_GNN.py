...
class GraphGNN(nn.Module):
    def __init__(self, device, edge_index, edge_attr, in_dim, out_dim, wind_mean=None, wind_std=None):
    ...
    def forward(self, x, label=None):
        ...
        edge_attr_norm = edge_attr_norm[None, :, :].repeat(node_src.size(0), 1, 1).to(self.device)
        # out = torch.cat([node_src, node_target, edge_attr_norm, edge_weight[:,:,None]], dim=-1)
        out = torch.cat([node_src, node_target, edge_attr_norm], dim=-1)
...
class PM25_GNN(BaseModel):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index, edge_attr, 
                 wind_mean=None, wind_std=None, **args):
        super(PM25_GNN, self).__init__(**args)
    ...
    def forward(self, pm25_hist, feature):
        ...
        for i in range(self.pred_len):
            x = torch.cat((xn, feature[:, i]), dim=-1)
