...
class BigST(BaseModel):
    def __init__(self, seq_num, in_dim, hid_dim, tau, random_feature_dim, node_emb_dim, time_emb_dim, \
                 use_residual, use_bn, use_spatial, use_long, dropout, tod, supports=None, edge_indices=None, **args):
        super(BigST, self).__init__(**args)
        ...
        if self.use_long:
            self.regression_layer = nn.Conv2d(hid_dim*4*2+hid_dim+seq_num, self.horizon, kernel_size=(1, 1), bias=True)
        else:
            self.regression_layer = nn.Conv2d(hid_dim*4*2, self.horizon, kernel_size=(1, 1), bias=True)

    def forward(self, x, label=None, feat=None):
        x = x.transpose(1, 2)
        # input: (B, N, T, D)
        B, N, T, D = x.size()
        
        time_emb = self.time_emb_layer[(x[:, :, -1, -2]*self.time_num).type(torch.LongTensor)]
        week_emb = self.week_emb_layer[(x[:, :, -1, -1]*self.week_num).type(torch.LongTensor)]
        ...
        if self.use_spatial:
            ...
            return (x, s_loss) if self.training else x
        else:
            x = x.transpose(1, 2).unsqueeze(-1)
            return (x, 0) if self.training else x
