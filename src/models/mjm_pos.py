...
class STID(BaseModel):
    def __init__(self, **args):
        ...
        if self.pos_per_node:
            self.adaptive_pos = nn.Parameter(
                torch.empty(1, self.embed_dim, self.num_nodes, 1))
        else:
            self.adaptive_pos = nn.Parameter(
                torch.empty(1, self.embed_dim, 1, 1))
        nn.init.xavier_uniform_(self.adaptive_pos)
        
        self.pos_ln = nn.LayerNorm(self.embed_dim)

        self.time_series_emb_layer_time = nn.Conv2d(
            in_channels=self.input_len, out_channels=self.embed_dim, kernel_size=1, bias=True)
        self.pos_ln_time = nn.LayerNorm(self.embed_dim)
    ...
    def forward(self, history_data, label=None, adj=None):
        ...
        pos = self.pos_ln(self.adaptive_pos.transpose(1, -1)).transpose(1, -1)

        time_series_emb = (seasonal_output + trend_output).unsqueeze(-1) \
            + pos \
            + self.pos_ln_time(self.time_series_emb_layer_time(input_data[..., 1:2]).transpose(1, -1)).transpose(1, -1)
