...
class SparseTSF(BaseModel):
    ...
    def __init__(self, model_args, **args):
        super(SparseTSF, self).__init__(**args)

        # get parameters
        self.seq_len = model_args.seq_len
        self.pred_len = model_args.horizon
        # self.enc_in = model_args['enc_in']
        self.enc_in = self.node_num
        self.period_len = model_args.tod
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor=None) -> torch.Tensor:
        x = history_data[:, :, :, 0]
