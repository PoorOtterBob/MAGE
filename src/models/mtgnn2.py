...
class MTGNN(BaseModel):
    ...
    def __init__(self, gcn_true=True, buildA_true=True, gcn_depth=2, 
                 # num_nodes, 
                 predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, 
                 # seq_length=12, 
                 # in_dim=2, 
                 # out_dim=12, 
    ...
    def forward(self, history_data: torch.Tensor, label=None, idx: int = None) -> torch.Tensor:
...
class graph_constructor(nn.Module):
    def __init__(self, nnodes, k, dim, alpha=3, static_feat=None):
        ...
        self.adj = nn.Parameter(self.generate_upper_triangle_matrix(self.nnodes), requires_grad=False)
    ...
    def generate_upper_triangle_matrix(self, N):
        upper_triangle_matrix = torch.zeros((N, N))

        # 填充上三角矩阵
        for i in range(N):
            # 计算当前行的值
            value = 1 / (N - i - 1) if (N - i - 1) != 0 else 0
            upper_triangle_matrix[i, i + 1:] = value  # 填充上三角部分（不包括对角线）
