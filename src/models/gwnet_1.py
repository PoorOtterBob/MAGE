...
class GWNET(BaseModel):
    ...
    def __init__(self, supports, adp_adj, dropout, residual_channels, dilation_channels, \
        ...
        if adp_adj:
            self.nodevec1 = nn.Parameter(torch.randn(blocks*layers, self.node_num, 10), requires_grad=True)
            self.nodevec2 = nn.Parameter(torch.randn(blocks*layers, 10, self.node_num), requires_grad=True)
            self.supports_len += 1
    ...
    def forward(self, input, label=None):  # (b, t, n, f)
        ...
        for i in range(self.blocks * self.layers):
            ...
            if self.adp_adj:
                adp = F.softmax(F.relu(torch.mm(self.nodevec1[i], self.nodevec2[i])), dim=1)
