...
class MAGE(BaseModel):
    ...
    def forward(self, x, label=None):
        h = x = self.encoder(self.channel_compress(x)) + self.prompt(x)
        z = x = self.backbone1(x)
        x = self.ffn(x)
        x = self.backbone2(h - x)
...
class MAGE_Block(nn.Module):
    def __init__(self, dim, expert_num, node_num, depth, model_dim, graph_gen_dim=32, head=4, 
                 expand_ratio=1, dropout=0):
        super(MAGE_Block, self).__init__()
        # Differential
        self.E1 = nn.Parameter(torch.randn(2, expert_num, node_num, graph_gen_dim))
        self.E2 = nn.Parameter(torch.randn(2, expert_num, graph_gen_dim, node_num))
        self.router = nn.Sequential(
            nn.Linear(dim, expert_num),
            nn.Softmax(dim=-1)
        )
        
        self.filter = nn.Parameter(torch.zeros(2, expert_num, node_num))
        ...
        self.linear = nn.Linear(dim, expand_ratio*dim)
        self.rank_holder = nn.Linear(dim, expand_ratio*dim)
        self.outer = nn.Linear(expand_ratio*dim, dim)
    ...
    def forward(self, x):
        ...
        x = torch.einsum('bik, e, ekid, ekj, ekdj, bjf -> bif', 
                         self.router(x),
                         self.cal_coefficient(),
                         torch.softmax(self.E1, dim=-1),
                         torch.softmax(self.filter, dim=-1),
                         torch.softmax(self.E2, dim=-1),
                         self.linear(x))*self.rank_holder(x)
        return self.outer(self.dropout(x))
