...
class MAGE_Block(nn.Module):
    ...
    def forward(self, x, bias):
        ...
        '''kv_buffer = torch.matmul(torch.softmax(self.E2, dim=-1).unsqueeze(2), # (e, k, 1, d, n)
                                 torch.matmul(x, self.cal_outer()) # (b, n, f)
                                 ) # (e, k, b, d, f)
        q_buffer = torch.matmul(torch.softmax(self.E1, dim=-1).unsqueeze(2), # (e, k, 1, n, d)
                                 kv_buffer,  # (e, k, b, d, f)
                                 ) # (e, k, b, n, f)
        mage = q_buffer[0] - self.cal_lambda()*q_buffer[1] # (k, b, n, f)
        mage = torch.sum(mage * route.permute(2, 0, 1).unsqueeze(-1), dim=0)
        return mage, topk_indices'''
