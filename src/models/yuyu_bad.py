...
class series_decomp(nn.Module):
    ...
    def forward(self, x):
        ...
        res = x - moving_mean
        return res.unsqueeze(-1), moving_mean.unsqueeze(-1)
...
class STID(BaseModel):
    def __init__(self, **args):
        ...
        if self.if_time_in_day:
            self.time_in_day_emb = nn.Embedding(self.time_of_day_size, 
                                                self.temp_dim_tid)
            # nn.init.xavier_uniform_(self.time_in_day_emb)
        if self.if_day_in_week:
            self.day_in_week_emb = nn.Embedding(self.day_of_week_size,
                                                self.temp_dim_diw)
        ...
        self.time_series_emb_layer2 = nn.Conv1d(
            in_channels=self.input_len, out_channels=self.embed_dim, kernel_size=1, bias=True)
        '''
        self.time_series_emb_layer = nn.Linear(self.input_dim+1, self.embed_dim)
        ...
        '''
        self.regression_layer = nn.Linear(self.hidden_dim, self.output_dim)

    def forward(self, input_data, label=None, adj=None):
        # prepare data
        # input_data = input_data[..., range(self.input_dim)]
        batch_size, _, num_nodes, _ = input_data.shape # (b, t, n, f)

        # temporal embeddings
        tem_emb = []
        if self.if_time_in_day:
            t_i_d_data = input_data[..., 1]
            tem_emb.append(self.time_in_day_emb((t_i_d_data * self.time_of_day_size).long()))
        if self.if_day_in_week:
            d_i_w_data = input_data[..., 2]
            tem_emb.append(self.day_in_week_emb((d_i_w_data * self.day_of_week_size).long()))
        ...
        time_series_emb  = torch.cat([seasonal_init, 
                                      trend_init, 
                                      input_data[..., 1:]], 
                                      dim=-1)
        time_series_emb = self.time_series_emb_layer(time_series_emb) # (b, t, n, f)
        # seasonal_output = self.time_series_emb_layer1(seasonal_init)
        # trend_output = self.time_series_emb_layer2(trend_init)

        # time_series_emb = (seasonal_output + trend_output) # (b, t, n, f)
...
class STRIP(BaseModel):
    def __init__(self, model_args, stmodel, dim, core, **args):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(self.in_dim, 4*self.decoder_hidden_dim), 
            nn.GELU(),
            nn.Dropout(0.15),
            # nn.Linear(self.decoder_hidden_dim, self.horizon if self.inverse else self.output_dim),
            nn.Linear(4*self.decoder_hidden_dim, 1),  
        )
