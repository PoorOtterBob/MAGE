...
class Pre_Norm_Transformer(nn.Module):
    def __init__(self, dim, expert_num, node_num, blocknum, topk=4):
        ...
        self.attn_norm = Dynamic_Tanh(dim)

        self.ffn = SwiGLU_FFN(dim, dim)
        # self.ffn_norm = RMSNorm(dim)
        self.ffn_norm = Dynamic_Tanh(dim)
...
class Dynamic_Tanh(nn.Module):
    def __init__(self, d, alpha_init=1, bias=False):
        super(Dynamic_Tanh, self).__init__()
