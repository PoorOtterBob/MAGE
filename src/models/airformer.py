...
class AirEmbedding(nn.Module):
    ...
    def __init__(self):
        ...
        self.embed_day=nn.Embedding(7, 3) # a typo here but doesn't affect the results. this layer is actually for embedding 24 hours.
        self.embed_hour=nn.Embedding(24, 5) # a typo here but doesn't affect the results. this layer is actually for embedding 7 days.
...
class AirFormer(BaseModel):
    ...
    def __init__(self, device, 
                 ...
                 spatial_flag=False,  # whether to use DS-MSA
        ...
        self.start_conv = nn.Conv2d(in_channels=8-2+8, # self.input_dim,
        ...
        for b in range(blocks):
            ...
            if self.spatial_flag:
                ...
                self.residual_convs.append(nn.Conv2d(in_channels=hidden_channels,
                                                     out_channels=hidden_channels,
                                                     kernel_size=(1, 1)))
    ...
    def forward(self, inputs, label=None, supports=None):
        ...
        x = torch.cat((inputs[..., :6], x_embed), -1)
        ...
        if self.stochastic_flag:
            ...
            return (x_hat, x_rec, kl_loss) if self.training else x_hat
        else:
            num_blocks, B, C, N, T = d.shape
            d = d.permute(1, 0, 2, 3, 4).reshape(
                B, -1, N, T)  # [B, num_blocks*C, N, T]
