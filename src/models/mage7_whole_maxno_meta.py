...
class MAGE(BaseModel):
    def __init__(self, model_args, **args):
        ...
        self.meta_prompt = Meta_Prompt_Pool(self.args)
    ...
    def forward(self, x, label=None, bias=[0, 0, 0], meta=None):
        prompt = self.prompt(x) + self.meta_prompt(meta)
...
class Meta_Prompt_Pool(nn.Module):
    def __init__(self, model_args):
        super(Meta_Prompt_Pool, self).__init__()
        self.pool = nn.ModuleList()
        
        self.pool.append(nn.Embedding(26, model_args.model_dim))
        
        self.pool.append(nn.Embedding(7, model_args.model_dim))
        
        self.pool.append(nn.Embedding(4, model_args.model_dim))

    def forward(self, meta):
        prompt = 0
        for i in range(len(self.pool)):
            prompt += self.pool[i]((meta[..., 2+i]).long())
