...
class AGCRN(BaseModel):
    ...
    def __init__(self, embed_dim, rnn_unit, num_layer, cheb_k, **args):
        ...
        self.node_embed = nn.Parameter(torch.randn(num_layer, 2, self.node_num, embed_dim), requires_grad=True)
    ...
    def forward(self, x, init_state, node_embed):
        ...
        for i in range(self.num_layer):
            ...
            for t in range(seq_length):
                state = self.dcrnn_cells[i](current_inputs[:, t, :, :], state, node_embed[i])
...
class AGCRNCell(nn.Module):
    ...
    def forward(self, x, state, node_embed):
        ...
        z_r = torch.sigmoid(self.gate(input_and_state, node_embed[0]))
        ...
        hc = torch.tanh(self.update(candidate, node_embed[1]))
        h = r*state + (1-r)*hc
        return h
