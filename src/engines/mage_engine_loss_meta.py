...
class BaseEngine():
    def __init__(self, device, model, dataloader, scaler, sampler, loss_fn, lrate, optimizer, \
        ...
        self._meta = self._to_tensor(meta).to(self._device)
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_r{}_bb{}_bn{}_dim{}.pt'.format(self._seed, 
    ...
    def train_batch(self):
        ...
        for X, label in tqdm(self._dataloader['train_loader'].get_iterator()):
            ...
            pred, topk_indices = self.model(X, None, self._bias, self._meta)
    ...
    def evaluate(self, mode):
        ...
        with torch.no_grad():
            for X, label in tqdm(self._dataloader[mode + '_loader'].get_iterator()):
                # X (b, t, n, f), label (b, t, n, 1)
                X, label = self._to_device(self._to_tensor([X, label]))
                pred = self.model(X, None, self._bias, self._meta)
                pred, label = self._inverse_transform([pred, label])
